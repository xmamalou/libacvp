...
static ACVP_RESULT acvp_lookup_prereqVals(JSON_Object *cap_obj,
    ...
    if (!cap_entry->has_prereq) {
        return ACVP_SUCCESS;
    }
    // Init json array
    json_object_set_value(cap_obj, ACVP_PREREQ_OBJ_STR,
...
static ACVP_RESULT acvp_append_true_false_array(JSON_Object *parent,
...
static ACVP_RESULT acvp_build_hash_register_cap(JSON_Object *cap_obj,
                                                ACVP_CAPS_LIST *cap_entry) {
    ...
    json_object_set_string(cap_obj, "algorithm",
                           acvp_lookup_cipher_name(cap_entry->cipher));
    ...
    if (cap_entry->cipher == ACVP_HASH_SHA3_224 ||
        ...
        json_object_set_boolean(cap_obj, "inBit",
                                cap_entry->cap.hash_cap->in_bit);
        json_object_set_boolean(cap_obj, "inEmpty",
                                cap_entry->cap.hash_cap->in_empty);
    }
    ...
    if (cap_entry->cipher == ACVP_HASH_SHAKE_128 ||
        ...
        json_object_set_boolean(cap_obj, "outBit",
                                cap_entry->cap.hash_cap->out_bit);
        ...
        json_object_set_number(tmp_obj, "min",
                               cap_entry->cap.hash_cap->out_len.min);
        json_object_set_number(tmp_obj, "max",
                               cap_entry->cap.hash_cap->out_len.max);
        json_object_set_number(tmp_obj, "increment",
                               cap_entry->cap.hash_cap->out_len.increment);

        json_array_append_value(tmp_arr, tmp_val);
    } else {
        json_object_set_value(cap_obj, "messageLength",
                              json_value_init_array());
        ...
        json_object_set_number(msg_obj, "increment",
                               hash_cap->msg_length.increment);
        json_array_append_value(msg_array, msg_val);

        // Set the supported large data lengths
        if (cap_entry->cap.hash_cap->large_lens) {
            json_object_set_value(cap_obj, "performLargeDataTest",
                                  json_value_init_array());
...
static ACVP_RESULT acvp_build_hmac_register_cap(JSON_Object *cap_obj,
                                                ACVP_CAPS_LIST *cap_entry) {
    JSON_Array *temp_arr = NULL;
    ACVP_RESULT result;
    ...
    if (hmac_cap->key_len.increment != 0) {
        ...
        json_object_set_number(key_len_obj, "increment",
                               hmac_cap->key_len.increment);
    ...
    if (hmac_cap->mac_len.increment != 0) {
        ...
        json_object_set_number(mac_len_obj, "increment",
                               hmac_cap->mac_len.increment);
...
static ACVP_RESULT acvp_build_cmac_register_cap(JSON_Object *cap_obj,
                                                ACVP_CAPS_LIST *cap_entry) {
    JSON_Array *temp_arr = NULL, *capabilities_arr = NULL;
    JSON_Value *capabilities_val = NULL, *msg_len_val = NULL,
               *mac_len_val = NULL;
    JSON_Object *capabilities_obj = NULL, *msg_len_obj = NULL,
                *mac_len_obj = NULL;
    ...
    json_object_set_value(capabilities_obj, "direction",
                          json_value_init_array());
    temp_arr = json_object_get_array(capabilities_obj, "direction");
    if (!cap_entry->cap.cmac_cap->direction_gen &&
        !cap_entry->cap.cmac_cap->direction_ver) {
        json_value_free(capabilities_val);
        return ACVP_MISSING_ARG;
    }
    if (cap_entry->cap.cmac_cap->direction_gen) {
        json_array_append_string(temp_arr, "gen");
    }
    if (cap_entry->cap.cmac_cap->direction_ver) {
        json_array_append_string(temp_arr, "ver");
    }
    ...
    if (cap_entry->cap.cmac_cap->msg_len.increment != 0) {
        ...
        json_object_set_number(msg_len_obj, "increment",
                               cmac_cap->msg_len.increment);
    ...
    if (cap_entry->cipher == ACVP_CMAC_AES) {
        ...
        json_object_set_value(capabilities_obj, "keyLen",
                              json_value_init_array());
        ...
        json_object_set_value(capabilities_obj, "keyingOption",
                              json_value_init_array());
...
static ACVP_RESULT acvp_build_kmac_register_cap(JSON_Object *cap_obj,
                                                ACVP_CAPS_LIST *cap_entry) {
    JSON_Array *temp_arr = NULL;
    ...
    json_object_set_boolean(cap_obj, "hexCustomization",
                            cap_entry->cap.kmac_cap->hex_customization);
...
static ACVP_RESULT acvp_build_cshake_register_cap(JSON_Object *cap_obj,
                                                  ACVP_CAPS_LIST *cap_entry) {
    JSON_Array *temp_arr = NULL;
    ...
    if (cshake_cap->output_len.increment != 0) {
        ...
        json_object_set_number(output_len_obj, "min",
                               cshake_cap->output_len.min);
        json_object_set_number(output_len_obj, "max",
                               cshake_cap->output_len.max);
        json_object_set_number(output_len_obj, "increment",
                               cshake_cap->output_len.increment);
...
static ACVP_RESULT
acvp_build_sym_cipher_register_cap(JSON_Object *cap_obj,
                                   ACVP_CAPS_LIST *cap_entry) {
    ...
    if ((cap_entry->cipher == ACVP_AES_KW) ||
        (cap_entry->cipher == ACVP_AES_KWP) ||
    ...
    if ((cap_entry->cipher == ACVP_AES_CTR) ||
        (cap_entry->cipher == ACVP_TDES_CTR)) {
        json_object_set_boolean(cap_obj, "incrementalCounter",
                                sym_cap->ctr_incr);
        json_object_set_boolean(cap_obj, "overflowCounter",
                                sym_cap->ctr_ovrflw);
        json_object_set_boolean(cap_obj, "performCounterTests",
                                sym_cap->perform_ctr_tests);
    }

    // Set the IV generation source if applicable

    // For some reason, RFC3686 uses "ivGenMode" instead of "ivGen" here- may be
    // corrected spec-side later on
    const char *ivGenLabel;
    if (cap_entry->cipher == ACVP_AES_CTR &&
        sym_cap->conformance == ACVP_CONFORMANCE_RFC3686) {
    ...
    if (sym_cap->ptlen) {
        ...
        if (cap_entry->cipher != ACVP_AES_FF1 &&
            cap_entry->cipher != ACVP_AES_FF3) {
            json_object_set_value(cap_obj, "payloadLen",
                                  json_value_init_array());
            opts_arr = json_object_get_array(cap_obj, "payloadLen");
            tmp_val = json_value_init_object();
            tmp_obj = json_value_get_object(tmp_val);
    ...
    if (cap_entry->cipher == ACVP_AES_XTS) {
        ...
        json_object_set_boolean(cap_obj, "dataUnitLenMatchesPayload",
                                sym_cap->dulen_matches_paylen);
        if (!sym_cap->dulen_matches_paylen) {
            json_object_set_value(cap_obj, "dataUnitLen",
                                  json_value_init_array());
...
static const char *
acvp_lookup_drbg_mode_string(ACVP_DRBG_MODE_LIST *drbg_cap_mode) {
...
static ACVP_RESULT acvp_build_drbg_register_cap(JSON_Object *cap_obj,
                                                ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    ...
    json_object_set_value(cap_obj, "predResistanceEnabled",
                          json_value_init_array());
    ...
    json_object_set_boolean(cap_obj, "reseedImplemented",
                            cap->reseed_implemented);
    ...
    while (cap_mode_list) {
        ...
        while (cap_group_list) {
            ...
            if (cap_entry->cipher == ACVP_CTRDRBG) {
                json_object_set_boolean(capabilities_obj, "derFuncEnabled",
                                        cap_group->der_func_enabled);
            }
            // Set entropy range
            json_object_set_value(capabilities_obj, "entropyInputLen",
                                  json_value_init_array());
            ...
            if (!cap_group->entropy_len_step) {
                ...
                json_object_set_number(len_obj, "max",
                                       cap_group->entropy_len_max);
                json_object_set_number(len_obj, "min",
                                       cap_group->entropy_len_min);
                json_object_set_number(len_obj, "increment",
                                       cap_group->entropy_len_step);
                json_array_append_value(array, len_val);
            }

            json_object_set_value(capabilities_obj, "nonceLen",
                                  json_value_init_array());
            ...
            if (!cap_group->nonce_len_step) {
                ...
                len_obj = json_value_get_object(len_val);
                json_object_set_number(len_obj, "max",
                                       cap_group->nonce_len_max);
                json_object_set_number(len_obj, "min",
                                       cap_group->nonce_len_min);
                json_object_set_number(len_obj, "increment",
                                       cap_group->nonce_len_step);
                json_array_append_value(array, len_val);
            }

            json_object_set_value(capabilities_obj, "persoStringLen",
                                  json_value_init_array());
            ...
            if (!cap_group->perso_len_step) {
                ...
                len_obj = json_value_get_object(len_val);
                json_object_set_number(len_obj, "max",
                                       cap_group->perso_len_max);
                json_object_set_number(len_obj, "min",
                                       cap_group->perso_len_min);
                json_object_set_number(len_obj, "increment",
                                       cap_group->perso_len_step);
                json_array_append_value(array, len_val);
            }

            json_object_set_value(capabilities_obj, "additionalInputLen",
                                  json_value_init_array());
            array =
                json_object_get_array(capabilities_obj, "additionalInputLen");
            if (!cap_group->additional_in_len_step) {
                if (cap_group->additional_in_len_min) {
                    json_array_append_number(array,
                                             cap_group->additional_in_len_min);
                } else if (cap_group->additional_in_len_max) {
                    json_array_append_number(array,
                                             cap_group->additional_in_len_max);
                }
                if (!cap_group->additional_in_len_min &&
                    !cap_group->additional_in_len_max) {
                    json_array_append_number(array, 0);
                ...
                json_object_set_number(len_obj, "min",
                                       cap_group->additional_in_len_min);
                json_object_set_number(len_obj, "increment",
                                       cap_group->additional_in_len_step);
                json_array_append_value(array, len_val);
            }

            // Set DRBG Length
            json_object_set_number(capabilities_obj, "returnedBitsLen",
                                   cap_group->returned_bits_len);
...
static ACVP_RESULT acvp_lookup_rsa_primes(JSON_Object *cap_obj,
                                          ACVP_RSA_KEYGEN_CAP *rsa_cap) {
    ACVP_RESULT rv = ACVP_SUCCESS;
    JSON_Array *primes_array = NULL, *hash_array = NULL,
               *prime_test_array = NULL;
    const char *str = NULL;
...
static ACVP_RESULT
acvp_build_rsa_keygen_register_cap(JSON_Object *cap_obj,
                                   ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    ...
    json_object_set_boolean(cap_obj, "infoGeneratedByServer",
                            keygen_cap->info_gen_by_server);
    if (!keygen_cap->pub_exp_mode) {
        return ACVP_MISSING_ARG;
    }
    json_object_set_string(cap_obj, "pubExpMode",
                           keygen_cap->pub_exp_mode ==
                                   ACVP_RSA_PUB_EXP_MODE_FIXED
                               ? ACVP_RSA_PUB_EXP_MODE_FIXED_STR
                               : ACVP_RSA_PUB_EXP_MODE_RANDOM_STR);
    if (keygen_cap->pub_exp_mode == ACVP_RSA_PUB_EXP_MODE_FIXED) {
        json_object_set_string(cap_obj, "fixedPubExp",
                               (const char *)keygen_cap->fixed_pub_exp);
    }
    json_object_set_string(cap_obj, "keyFormat",
                           keygen_cap->key_format_crt ? "crt" : "standard");
    ...
    while (keygen_cap) {
        ...
        json_object_set_string(
            alg_specs_obj, "randPQ",
            acvp_lookup_rsa_randpq_name(keygen_cap->rand_pq));
...
static ACVP_RESULT acvp_build_rsa_sig_register_cap(JSON_Object *cap_obj,
                                                   ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result = ACVP_SUCCESS;
    ACVP_PARAM_LIST *plist = NULL;
    ACVP_RSA_SIG_CAP *rsa_cap_mode = NULL;
    ACVP_REVISION rev = ACVP_REVISION_DEFAULT;
    JSON_Array *alg_specs_array = NULL, *sig_type_caps_array = NULL,
               *hash_pair_array = NULL, *mask_func_array = NULL;
    JSON_Value *alg_specs_val = NULL, *sig_type_val = NULL,
               *hash_pair_val = NULL;
    JSON_Object *alg_specs_obj = NULL, *sig_type_obj = NULL,
                *hash_pair_obj = NULL;
    ...
    if (rsa_cap_mode->revision) {
        ...
        rev = rsa_cap_mode->revision; // Since caps are stringed together, we
    ...
    if (cap_entry->cipher == ACVP_RSA_SIGVER) {
        json_object_set_string(cap_obj, "pubExpMode",
                               rsa_cap_mode->pub_exp_mode ==
                                       ACVP_RSA_PUB_EXP_MODE_FIXED
                                   ? ACVP_RSA_PUB_EXP_MODE_FIXED_STR
                                   : ACVP_RSA_PUB_EXP_MODE_RANDOM_STR);
        if (rsa_cap_mode->pub_exp_mode == ACVP_RSA_PUB_EXP_MODE_FIXED) {
            json_object_set_string(cap_obj, "fixedPubExp",
                                   (const char *)rsa_cap_mode->fixed_pub_exp);
        }
    }
    ...
    while (rsa_cap_mode) {
        ...
        json_object_set_value(alg_specs_obj, "properties",
                              json_value_init_array());
        sig_type_caps_array =
            json_object_get_array(alg_specs_obj, "properties");

        ACVP_RSA_MODE_CAPS_LIST *current_sig_type_cap =
            rsa_cap_mode->mode_capabilities;

        while (current_sig_type_cap) {
            sig_type_val = json_value_init_object();
            sig_type_obj = json_value_get_object(sig_type_val);

            json_object_set_number(sig_type_obj, "modulo",
                                   current_sig_type_cap->modulo);

            if (rev != ACVP_REVISION_FIPS186_4 &&
                rsa_cap_mode->sig_type == ACVP_RSA_SIG_TYPE_PKCS1PSS) {
                json_object_set_value(sig_type_obj, "maskFunction",
                                      json_value_init_array());
                mask_func_array =
                    json_object_get_array(sig_type_obj, "maskFunction");
            ...
            json_object_set_value(sig_type_obj, "hashPair",
                                  json_value_init_array());
            hash_pair_array = json_object_get_array(sig_type_obj, "hashPair");

            ACVP_RSA_HASH_PAIR_LIST *current_hash_pair =
                current_sig_type_cap->hash_pair;
            while (current_hash_pair) {
                ...
                if (rsa_cap_mode->sig_type == ACVP_RSA_SIG_TYPE_PKCS1PSS) {
                    json_object_set_number(hash_pair_obj, "saltLen",
                                           current_hash_pair->salt);
                }
...
static ACVP_RESULT acvp_build_rsa_prim_register_cap(JSON_Object *cap_obj,
                                                    ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    ...
    if (cap_entry->cap.rsa_prim_cap->revision) {
        revision = acvp_lookup_alt_revision_string(
            cap_entry->cap.rsa_prim_cap->revision);
    } else {
        revision = acvp_lookup_cipher_revision(cap_entry->cipher);
    }
    if (revision == NULL)
    ...
    if (prim_cap->revision && cap_entry->cipher == ACVP_RSA_SIGPRIM) {
        ...
        if (pl) {
            json_object_set_string(cap_obj, "keyFormat",
                                   acvp_lookup_rsa_format_str(pl->param));
        }
    } else if (!prim_cap->revision) {
        pl = prim_cap->key_formats;
        if (pl) {
            json_object_set_value(cap_obj, "keyFormat",
                                  json_value_init_array());
    ...
    if (prim_cap->pub_exp_mode == ACVP_RSA_PUB_EXP_MODE_FIXED) {
        json_object_set_string(cap_obj, "fixedPubExp",
                               (const char *)prim_cap->fixed_pub_exp);
    }

    return ACVP_SUCCESS;
}

static ACVP_RESULT acvp_build_ecdsa_register_cap(ACVP_CTX *ctx,
                                                 ACVP_CIPHER cipher,
                                                 JSON_Object *cap_obj,
                                                 ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    JSON_Array *caps_arr = NULL, *curves_arr = NULL, *secret_modes_arr = NULL,
               *hash_arr = NULL;
    ...
    case ACVP_SUB_ECDSA_SIGGEN:
        ...
        current_curve = ecdsa_cap->curves;
        // add "universally" set hash algs here instead of later to be resilient
        // to different combos of API calls
        while (current_curve) {
    ...
    if (cipher == ACVP_ECDSA_KEYGEN) {
        json_object_set_value(cap_obj, "secretGenerationMode",
                              json_value_init_array());
        secret_modes_arr =
            json_object_get_array(cap_obj, "secretGenerationMode");
        while (current_secret_mode) {
            if (!current_secret_mode->name) {
                return ACVP_MISSING_ARG;
            }
            json_array_append_string(secret_modes_arr,
                                     current_secret_mode->name);
            current_secret_mode = current_secret_mode->next;
        }
    }

    /**
     * hashAlgs is relatively complicated. We want to compare every curve that
     * is registered and the hash algs registered to them. If they have the same
     * hash algs, we put them in the same object within the capabilities array.
     * If they have different hash algs, a new object is created.
     */
    if (cipher == ACVP_ECDSA_SIGGEN || cipher == ACVP_ECDSA_SIGVER ||
        ...
        while (current_curve) {
            ...
            // One of these vals for every object in the array - every object
            // being list of curves that share the same hashAlgs
            alg_caps_val = json_value_init_object();
            alg_caps_obj = json_value_get_object(alg_caps_val);

            json_object_set_value(alg_caps_obj, "curve",
                                  json_value_init_array());
            curves_arr = json_object_get_array(alg_caps_obj, "curve");
            json_object_set_value(alg_caps_obj, "hashAlg",
                                  json_value_init_array());
            ...
            // Now, check every curve on the list afterwards, and memcmp to see
            // if it has the same hashAlgs, appending it to the same obj when
            // applicable
            iter = current_curve->next;
            while (iter) {
                // If the curve is already accounted for by a previous
                // current_curve, skip
                if (current_curve->curve == iter->curve || track[iter->curve]) {
                    iter = iter->next;
                    continue;
                }
                memcmp_s(current_curve->algs, sizeof(current_curve->algs),
                         iter->algs, sizeof(iter->algs), &diff);
                if (!diff) {
                    // if they have the same algs arrays, they go in the same
                    // obj in the capabilities array
...
static ACVP_RESULT acvp_build_eddsa_register_cap(ACVP_CTX *ctx,
                                                 JSON_Object *cap_obj,
                                                 ACVP_CAPS_LIST *cap_entry) {
    ...
    if (alg == ACVP_SUB_EDDSA_SIGGEN) {
        ...
        json_object_set_number(obj, "increment",
                               eddsa_cap->context_len.increment);
...
static ACVP_RESULT
acvp_build_kdf135_snmp_register_cap(JSON_Object *cap_obj,
                                    ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    JSON_Array *temp_arr = NULL;
...
static ACVP_RESULT
acvp_build_kdf108_mode_register(JSON_Object **mode_obj,
                                ACVP_KDF108_MODE_PARAMS *mode_params) {
    JSON_Array *tmp_arr = NULL;
    JSON_Value *tmp_val = NULL;
    JSON_Object *tmp_obj = NULL;
    ...
    json_object_set_value(*mode_obj, "supportedLengths",
                          json_value_init_array());
    ...
    if (mode_params->supported_lens.increment != 0) {
        ...
        json_object_set_number(tmp_obj, "increment",
                               mode_params->supported_lens.increment);
        json_array_append_value(tmp_arr, tmp_val);
    }
    ...
    json_object_set_boolean(*mode_obj, "supportsEmptyIv",
                            mode_params->empty_iv_support);
    if (mode_params->empty_iv_support) {
        json_object_set_boolean(*mode_obj, "requiresEmptyIv",
                                mode_params->requires_empty_iv);
    }

    return ACVP_SUCCESS;
}

static ACVP_RESULT acvp_build_kdf108_register_cap(JSON_Object *cap_obj,
                                                  ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    JSON_Array *alg_specs_array = NULL;
    JSON_Value *alg_specs_counter_val = NULL, *alg_specs_feedback_val = NULL,
               *alg_specs_dpi_val = NULL;
    JSON_Object *alg_specs_counter_obj = NULL, *alg_specs_feedback_obj = NULL,
                *alg_specs_dpi_obj = NULL;
    const char *revision = NULL;
    ...
    if (cap_entry->cap.kdf108_cap->counter_mode.kdf_mode) {
        ...
        acvp_build_kdf108_mode_register(
            &alg_specs_counter_obj, &cap_entry->cap.kdf108_cap->counter_mode);
    ...
    if (cap_entry->cap.kdf108_cap->feedback_mode.kdf_mode) {
        ...
        acvp_build_kdf108_mode_register(
            &alg_specs_feedback_obj, &cap_entry->cap.kdf108_cap->feedback_mode);
    ...
    if (cap_entry->cap.kdf108_cap->dpi_mode.kdf_mode) {
        ...
        acvp_build_kdf108_mode_register(&alg_specs_dpi_obj,
                                        &cap_entry->cap.kdf108_cap->dpi_mode);
...
static ACVP_RESULT
acvp_build_kdf108_kmac_register_cap(JSON_Object *cap_obj,
                                    ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    JSON_Array *tmp_arr = NULL;
    JSON_Value *tmp_val = NULL;
    ...
    // key derivation key length list
    json_object_set_value(cap_obj, "keyDerivationKeyLength",
                          json_value_init_array());
    ...
    json_object_set_number(tmp_obj, "increment",
                           params->derivation_keylens.increment);
    json_array_append_value(tmp_arr, tmp_val);
    ...
    json_object_set_number(tmp_obj, "increment",
                           params->derived_keylens.increment);
    json_array_append_value(tmp_arr, tmp_val);
...
static ACVP_RESULT
acvp_build_kdf135_x942_register_cap(JSON_Object *cap_obj,
                                    ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    JSON_Array *tmp_arr = NULL;
    JSON_Value *tmp_val = NULL;
    ...
    // Array of OIDs
    if (cap->type == ACVP_KDF_X942_KDF_TYPE_DER ||
        cap->type == ACVP_KDF_X942_KDF_TYPE_BOTH) {
...
static ACVP_RESULT
acvp_build_kdf135_x963_register_cap(JSON_Object *cap_obj,
                                    ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    JSON_Array *tmp_arr = NULL;
    ACVP_NAME_LIST *nl_obj;
...
static ACVP_RESULT
acvp_build_kdf135_ikev2_register_cap(JSON_Object *cap_obj,
                                     ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    ...
    // initiator nonce len
    json_object_set_value(alg_specs_obj, "initiatorNonceLength",
                          json_value_init_array());
    ...
    if (cap->init_nonce_len_domain.increment != 0) {
        ...
        json_object_set_number(tmp_obj, "increment",
                               cap->init_nonce_len_domain.increment);
        json_array_append_value(tmp_arr, tmp_val);
    }
    ...
    // responder nonce len
    json_object_set_value(alg_specs_obj, "responderNonceLength",
                          json_value_init_array());
    ...
    if (cap->respond_nonce_len_domain.increment != 0) {
        ...
        tmp_obj = json_value_get_object(tmp_val);
        json_object_set_number(tmp_obj, "min",
                               cap->respond_nonce_len_domain.min);
        json_object_set_number(tmp_obj, "max",
                               cap->respond_nonce_len_domain.max);
        json_object_set_number(tmp_obj, "increment",
                               cap->respond_nonce_len_domain.increment);
        json_array_append_value(tmp_arr, tmp_val);
    }
    ...
    json_object_set_value(alg_specs_obj, "diffieHellmanSharedSecretLength",
                          json_value_init_array());
    tmp_arr =
        json_object_get_array(alg_specs_obj, "diffieHellmanSharedSecretLength");
    ...
    json_object_set_value(alg_specs_obj, "derivedKeyingMaterialLength",
                          json_value_init_array());
    tmp_arr =
        json_object_get_array(alg_specs_obj, "derivedKeyingMaterialLength");
    ...
    if (cap->key_material_len.increment != 0) {
        ...
        json_object_set_number(tmp_obj, "increment",
                               cap->key_material_len.increment);
        json_array_append_value(tmp_arr, tmp_val);
    }
...
static ACVP_RESULT
acvp_build_kdf135_ikev1_register_cap(JSON_Object *cap_obj,
                                     ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    ...
    json_object_set_number(
        tmp_obj, "min",
        cap_entry->cap.kdf135_ikev1_cap->init_nonce_len_domain.min);
    json_object_set_number(
        tmp_obj, "max",
        cap_entry->cap.kdf135_ikev1_cap->init_nonce_len_domain.max);
    json_object_set_number(
        tmp_obj, "increment",
        cap_entry->cap.kdf135_ikev1_cap->init_nonce_len_domain.increment);
    json_array_append_value(tmp_arr, tmp_val);
    ...
    json_object_set_number(
        tmp_obj, "min",
        cap_entry->cap.kdf135_ikev1_cap->respond_nonce_len_domain.min);
    json_object_set_number(
        tmp_obj, "max",
        cap_entry->cap.kdf135_ikev1_cap->respond_nonce_len_domain.max);
    json_object_set_number(
        tmp_obj, "increment",
        cap_entry->cap.kdf135_ikev1_cap->respond_nonce_len_domain.increment);
    json_array_append_value(tmp_arr, tmp_val);
    ...
    tmp_obj = json_value_get_object(tmp_val);
    json_object_set_number(tmp_obj, "min",
                           cap_entry->cap.kdf135_ikev1_cap->dh_secret_len.min);
    json_object_set_number(tmp_obj, "max",
                           cap_entry->cap.kdf135_ikev1_cap->dh_secret_len.max);
    json_object_set_number(
        tmp_obj, "increment",
        cap_entry->cap.kdf135_ikev1_cap->dh_secret_len.increment);
    json_array_append_value(tmp_arr, tmp_val);

    // Pre shared key len
    json_object_set_value(alg_specs_obj, "preSharedKeyLength",
                          json_value_init_array());
    tmp_arr = json_object_get_array(alg_specs_obj, "preSharedKeyLength");
    tmp_val = json_value_init_object();
    tmp_obj = json_value_get_object(tmp_val);
    json_object_set_number(tmp_obj, "min",
                           cap_entry->cap.kdf135_ikev1_cap->psk_len.min);
    json_object_set_number(tmp_obj, "max",
                           cap_entry->cap.kdf135_ikev1_cap->psk_len.max);
    json_object_set_number(tmp_obj, "increment",
                           cap_entry->cap.kdf135_ikev1_cap->psk_len.increment);
    json_array_append_value(tmp_arr, tmp_val);
    ...
    json_object_set_string(alg_specs_obj, "authenticationMethod",
                           cap_entry->cap.kdf135_ikev1_cap->auth_method);
...
static ACVP_RESULT
acvp_build_kdf135_srtp_register_cap(JSON_Object *cap_obj,
                                    ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT result;
    JSON_Array *tmp_arr = NULL;
    int i;
    ...
    json_object_set_boolean(cap_obj, "supportsZeroKdr",
                            cap_entry->cap.kdf135_srtp_cap->supports_zero_kdr);
...
static ACVP_RESULT
acvp_build_kdf135_ssh_register_cap(JSON_Object *cap_obj,
                                   ACVP_CAPS_LIST *cap_entry) {
    JSON_Array *temp_arr = NULL;
    ACVP_RESULT result;
    const char *revision = NULL;
...
static ACVP_RESULT acvp_build_pbkdf_register_cap(JSON_Object *cap_obj,
                                                 ACVP_CAPS_LIST *cap_entry) {
    ...
    // create the "iterationCount" array within the "capabilities" array and
    // populate it
    json_object_set_value(cap_sub_obj, "iterationCount",
                          json_value_init_array());
    temp_arr = json_object_get_array(cap_sub_obj, "iterationCount");
    tmp_val = json_value_init_object();
    tmp_obj = json_value_get_object(tmp_val);
    json_object_set_number(
        tmp_obj, "min", cap_entry->cap.pbkdf_cap->iteration_count_domain.min);
    json_object_set_number(
        tmp_obj, "max", cap_entry->cap.pbkdf_cap->iteration_count_domain.max);
    json_object_set_number(
        tmp_obj, "increment",
        cap_entry->cap.pbkdf_cap->iteration_count_domain.increment);
    ...
    tmp_obj = json_value_get_object(tmp_val);
    json_object_set_number(tmp_obj, "min",
                           cap_entry->cap.pbkdf_cap->key_len_domain.min);
    json_object_set_number(tmp_obj, "max",
                           cap_entry->cap.pbkdf_cap->key_len_domain.max);
    json_object_set_number(tmp_obj, "increment",
                           cap_entry->cap.pbkdf_cap->key_len_domain.increment);
    json_array_append_value(temp_arr, tmp_val);

    // create the "passwordLen" array within the "capabilities" array and
    // populate it
    ...
    tmp_obj = json_value_get_object(tmp_val);
    json_object_set_number(tmp_obj, "min",
                           cap_entry->cap.pbkdf_cap->password_len_domain.min);
    json_object_set_number(tmp_obj, "max",
                           cap_entry->cap.pbkdf_cap->password_len_domain.max);
    json_object_set_number(
        tmp_obj, "increment",
        cap_entry->cap.pbkdf_cap->password_len_domain.increment);
    json_array_append_value(temp_arr, tmp_val);

    // create the "saltLen" array within the "capabilities" array and populate
    // it
    ...
    tmp_obj = json_value_get_object(tmp_val);
    json_object_set_number(tmp_obj, "min",
                           cap_entry->cap.pbkdf_cap->salt_len_domain.min);
    json_object_set_number(tmp_obj, "max",
                           cap_entry->cap.pbkdf_cap->salt_len_domain.max);
    json_object_set_number(tmp_obj, "increment",
                           cap_entry->cap.pbkdf_cap->salt_len_domain.increment);
    json_array_append_value(temp_arr, tmp_val);

    // create the "hmacAlg" array within the "capabilities" array and populate
    // it
...
static ACVP_RESULT
acvp_build_kdf_tls12_register_cap(JSON_Object *cap_obj,
                                  ACVP_CAPS_LIST *cap_entry) {
    JSON_Array *temp_arr = NULL;
    ACVP_RESULT result;
...
static ACVP_RESULT
acvp_build_kdf_tls13_register_cap(JSON_Object *cap_obj,
                                  ACVP_CAPS_LIST *cap_entry) {
    JSON_Array *temp_arr = NULL;
    ACVP_NAME_LIST *hmac_alg_list = NULL;
...
static ACVP_RESULT acvp_build_dsa_pqggen_register(JSON_Array *meth_array,
    ...
    dsa_cap_mode =
        &cap_entry->cap.dsa_cap->dsa_cap_mode[ACVP_DSA_MODE_PQGGEN - 1];
...
static ACVP_RESULT acvp_build_dsa_pqgver_register(JSON_Array *meth_array,
    ...
    dsa_cap_mode =
        &cap_entry->cap.dsa_cap->dsa_cap_mode[ACVP_DSA_MODE_PQGVER - 1];
    attrs = dsa_cap_mode->dsa_attrs;
...
static ACVP_RESULT acvp_build_dsa_keygen_register(JSON_Array *meth_array,
    ...
    dsa_cap_mode =
        &cap_entry->cap.dsa_cap->dsa_cap_mode[ACVP_DSA_MODE_KEYGEN - 1];
    attrs = dsa_cap_mode->dsa_attrs;
...
static ACVP_RESULT acvp_build_dsa_siggen_register(JSON_Array *meth_array,
    ...
    dsa_cap_mode =
        &cap_entry->cap.dsa_cap->dsa_cap_mode[ACVP_DSA_MODE_SIGGEN - 1];
    attrs = dsa_cap_mode->dsa_attrs;
...
static ACVP_RESULT acvp_build_dsa_sigver_register(JSON_Array *meth_array,
    ...
    dsa_cap_mode =
        &cap_entry->cap.dsa_cap->dsa_cap_mode[ACVP_DSA_MODE_SIGVER - 1];
    attrs = dsa_cap_mode->dsa_attrs;
...
static ACVP_RESULT acvp_build_ascon_hash256_cap(ACVP_CTX *ctx,
    ...
    ACVP_LOG_INFO("    TagLen: { min = %d, max = %d, increment = %d}",
                  cap->msg_len.min, cap->msg_len.max, cap->msg_len.increment);
...
static ACVP_RESULT
acvp_lookup_kas_ecc_prereqVals(JSON_Object *cap_obj,
    ...
    if (kas_ecc_mode->prereq_vals) {
        ...
        case ACVP_KAS_ECC_MODE_CDH:
            if (kas_ecc_mode->function) {
                json_object_set_value(cap_obj, "function",
                                      json_value_init_array());
                ...
                while (current_func) {
                    ...
                    case ACVP_KAS_ECC_FUNC_FULL:
                        ...
                        break;
                    default:
                        ACVP_LOG_ERR("Unsupported KAS-ECC function %d",
                                     current_func->param);
                        return ACVP_INVALID_ARG;
            ...
            while (current_curve) {
                ...
                curve_str = acvp_lookup_ec_curve_name(kas_ecc_cap->cipher,
                                                      current_curve->param);
                if (!curve_str) {
                    ACVP_LOG_ERR("Unsupported curve %d", current_curve->param);
                    return ACVP_INVALID_ARG;
                }
        ...
        case ACVP_KAS_ECC_MODE_NONE:
            ...
            while (current_scheme) {
                ...
                json_object_set_value(func_obj, "kasRole",
                                      json_value_init_array());
                ...
                while (role) {
                    ...
                    case ACVP_KAS_ECC_ROLE_RESPONDER:
                        ...
                        break;
                    default:
                        ACVP_LOG_ERR("Unsupported KAS-ECC role %d",
                                     role->param);
                        return ACVP_INVALID_ARG;
                ...
                case ACVP_KAS_ECC_EPHEMERAL_UNIFIED:
                    json_object_set_value(sch_obj, "ephemeralUnified",
                                          func_val);
                    break;
            ...
            json_object_set_value(cap_obj, "domainParameterGenerationMethods",
                                  json_value_init_array());
            temp_arr = json_object_get_array(
                cap_obj, "domainParameterGenerationMethods");
            current_curve = kas_ecc_mode->curve;
            while (current_curve) {
                const char *curve_str = NULL;
            ...
            case ACVP_SHA512_224:
                json_object_set_string(cap_obj, "hashFunctionZ",
                                       "SHA2-512/224");
                break;
            ...
            case ACVP_NO_SHA:
                break;
            default:
                ACVP_LOG_ERR("Unsupported KAS-ECC sha param %d",
                             kas_ecc_mode->hash);
                return ACVP_INVALID_ARG;
                break;
            }
        ...
        case ACVP_KAS_ECC_MODE_COMPONENT:
            ...
            while (current_scheme) {
                ...
                while (current_pset) {
                    ...
                    json_object_set_value(set_obj, "hashAlg",
                                          json_value_init_array());
...
static ACVP_RESULT
acvp_lookup_kas_ffc_prereqVals(JSON_Object *cap_obj,
        ...
        case ACVP_KAS_FFC_MODE_COMPONENT:
            ...
            while (current_func) {
                ...
                case ACVP_KAS_FFC_FUNC_FULL:
                    ...
                    break;
                default:
                    ACVP_LOG_ERR("Unsupported KAS-FFC function %d",
                                 current_func->param);
                    return ACVP_INVALID_ARG;
            ...
            while (current_scheme) {
                ...
                while (current_pset) {
                    ...
                    while (sha) {
                        ...
                        case ACVP_SHA512:
                            ...
                            break;
                        default:
                            ACVP_LOG_ERR("Unsupported KAS-FFC sha param %d",
                                         sha->param);
                            return ACVP_INVALID_ARG;
                ...
                while (role) {
                    ...
                    case ACVP_KAS_FFC_ROLE_RESPONDER:
                        ...
                        break;
                    default:
                        ACVP_LOG_ERR("Unsupported KAS-FFC role %d",
                                     role->param);
                        return ACVP_INVALID_ARG;
...
static ACVP_RESULT acvp_build_kas_ifc_register_cap(ACVP_CTX *ctx,
    ...
    json_object_set_string(cap_obj, "fixedPubExp",
                           (const char *)kas_ifc_cap->fixed_pub_exp);
    ...
    json_object_set_value(cap_obj, "keyGenerationMethods",
                          json_value_init_array());
    ...
    while (current_param) {
        ...
        case ACVP_KAS_IFC_RSAKPG2_CRT:
            ...
            break;
        default:
            ACVP_LOG_ERR("Unsupported KAS-IFC keygen param %d",
                         current_param->param);
            return ACVP_INVALID_ARG;
            break;
        }
    ...
    if (current_param) {
        ...
        while (current_param) {
            ...
            case ACVP_KAS_IFC_RESPONDER:
                ...
                break;
            default:
                ACVP_LOG_ERR("Unsupported KAS-IFC KAS1 role param %d",
                             current_param->param);
                return ACVP_INVALID_ARG;
                break;
            }
    ...
    if (current_param) {
        ...
        while (current_param) {
            ...
            case ACVP_KAS_IFC_RESPONDER:
                ...
                break;
            default:
                ACVP_LOG_ERR("Unsupported KAS-IFC KAS2 role param %d",
                             current_param->param);
                return ACVP_INVALID_ARG;
                break;
            }
...
static ACVP_RESULT
acvp_build_kda_onestep_register_cap(ACVP_CTX *ctx, JSON_Object *cap_obj,
                                    ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT rv = ACVP_SUCCESS;
    ...
    if (!mode) {
        ACVP_LOG_ERR("Unable to find mode string for KDA-ONESTEP when building "
                     "registration");
    ...
    if (cap_entry->cap.kda_onestep_cap->revision) {
        revision = acvp_lookup_alt_revision_string(
            cap_entry->cap.kda_onestep_cap->revision);
    } else {
        revision = acvp_lookup_cipher_revision(cap_entry->cipher);
    }
    if (!revision) {
        ACVP_LOG_ERR("Unable to find revision string for KDA-ONESTEP when "
    ...
    while (tmp_param_list) {
        ...
        case ACVP_KDA_PATTERN_LITERAL:
            if (!cap_entry->cap.kda_onestep_cap->literal_pattern_candidate) {
                ACVP_LOG_ERR(
                    "Missing literal pattern candidate for registration");
                rv = ACVP_MISSING_ARG;
                goto err;
            }
    ...
    while (tmp_param_list) {
        ...
        case ACVP_KDA_ENCODING_CONCAT:
            json_array_append_string(temp_arr,
                                     ACVP_KDA_ENCODING_CONCATENATION_STR);
            break;
        default:
    ...
    while (tmp_name_list) {
        ...
        json_object_set_value(tmp_obj, "macSaltMethods",
                              json_value_init_array());
    ...
    tmp_obj = json_value_get_object(tmp_val);
    json_object_set_number(tmp_obj, "min",
                           cap_entry->cap.kda_onestep_cap->z.min);
    json_object_set_number(tmp_obj, "max",
                           cap_entry->cap.kda_onestep_cap->z.max);
    json_object_set_number(tmp_obj, "increment",
                           cap_entry->cap.kda_onestep_cap->z.increment);
    json_array_append_value(temp_arr, tmp_val);
err:
    if (pattern_str)
        free(pattern_str);
...
static ACVP_RESULT
acvp_build_kda_twostep_register_cap(ACVP_CTX *ctx, JSON_Object *cap_obj,
                                    ACVP_CAPS_LIST *cap_entry) {
    ACVP_RESULT rv = ACVP_SUCCESS;
    ...
    mode = acvp_lookup_cipher_mode_str(cap_entry->cipher);
    if (!mode) {
        ACVP_LOG_ERR("Unable to find mode string for KDA-TWOSTEP when building "
                     "registration");
        rv = ACVP_INVALID_ARG;
        goto err;
    }
    ...
    if (!revision) {
        ACVP_LOG_ERR("Unable to find revision string for KDA-TWOSTEP when "
                     "building registration");
        rv = ACVP_INVALID_ARG;
        goto err;
    }
    ...
    if (cap->revision != ACVP_REVISION_SP800_56CR1) {
        json_object_set_boolean(cap_obj, "performMultiExpansionTests",
                                cap->perform_multi_expansion_tests);
    }

    // Append the "usesHybridShareSecret" value and "auxSharedSecretLen" value
    // if enabled
    if (cap_entry->cap.kda_twostep_cap->use_hybrid_shared_secret) {
        json_object_set_boolean(cap_obj, "usesHybridSharedSecret", 1);
        json_object_set_value(cap_obj, "auxSharedSecretLen",
                              json_value_init_array());
        ...
        if (cap_entry->cap.kda_twostep_cap->aux_secret_len.min != 0 ||
            ...
            json_object_set_number(
                tmp_obj, "min",
                cap_entry->cap.kda_twostep_cap->aux_secret_len.min);
            json_object_set_number(
                tmp_obj, "max",
                cap_entry->cap.kda_twostep_cap->aux_secret_len.max);
            json_object_set_number(
                tmp_obj, "increment",
                cap_entry->cap.kda_twostep_cap->aux_secret_len.increment);
            json_array_append_value(tmp_arr, tmp_val);
        }
    ...
    /* Make an object with all of the common parameters in it. Then, copy it for
    each mode and add mode-specific stuff */
    ...
    while (tmp_param_list) {
        switch (tmp_param_list->param) {
        case ACVP_KDA_ENCODING_CONCAT:
            json_array_append_string(tmp_arr,
                                     ACVP_KDA_ENCODING_CONCATENATION_STR);
            break;
        default:
            ACVP_LOG_ERR("Invalid encoding value in encoding list");
    ...
    json_object_set_value(common_obj, "macSaltMethods",
                          json_value_init_array());
    ...
    if (cap->kdf_params.counter_mode.kdf_mode) {
        ...
        alg_specs_counter_obj = json_value_get_object(alg_specs_counter_val);
        json_object_set_string(alg_specs_counter_obj, "kdfMode", "counter");
        acvp_build_kdf108_mode_register(&alg_specs_counter_obj,
                                        &cap->kdf_params.counter_mode);
        json_array_append_value(alg_specs_array, alg_specs_counter_val);
    }
    if (cap->kdf_params.feedback_mode.kdf_mode) {
        ...
        alg_specs_feedback_obj = json_value_get_object(alg_specs_feedback_val);
        json_object_set_string(alg_specs_feedback_obj, "kdfMode", "feedback");
        acvp_build_kdf108_mode_register(&alg_specs_feedback_obj,
                                        &cap->kdf_params.feedback_mode);
        json_array_append_value(alg_specs_array, alg_specs_feedback_val);
    }
...
static ACVP_RESULT acvp_build_kda_hkdf_register_cap(ACVP_CTX *ctx,
    ...
    mode = acvp_lookup_cipher_mode_str(cap_entry->cipher);
    if (!mode) {
        ACVP_LOG_ERR("Unable to find mode string for KDA-HKDF when building "
                     "registration");
        rv = ACVP_INVALID_ARG;
        goto err;
    }
    ...
    if (!revision) {
        ACVP_LOG_ERR("Unable to find revision string for KDA-HKDF when "
                     "building registration");
        rv = ACVP_INVALID_ARG;
        goto err;
    }
    ...
    if (cap_entry->cap.kda_hkdf_cap->use_hybrid_shared_secret) {
        ...
        if (cap_entry->cap.kda_hkdf_cap->aux_secret_len.min != 0 ||
            ...
            json_object_set_number(
                tmp_obj, "min",
                cap_entry->cap.kda_hkdf_cap->aux_secret_len.min);
            json_object_set_number(
                tmp_obj, "max",
                cap_entry->cap.kda_hkdf_cap->aux_secret_len.max);
            json_object_set_number(
                tmp_obj, "increment",
                cap_entry->cap.kda_hkdf_cap->aux_secret_len.increment);
            json_array_append_value(temp_arr, tmp_val);
        }
...
static ACVP_RESULT acvp_build_kts_ifc_register_cap(ACVP_CTX *ctx,
    ...
    json_object_set_string(cap_obj, "fixedPubExp",
                           (const char *)kts_ifc_cap->fixed_pub_exp);
    ...
    while (current_param) {
        ...
        case ACVP_KTS_IFC_RSAKPG2_CRT:
            ...
            break;
        default:
            ACVP_LOG_ERR("Unsupported KTS-IFC keygen param %d",
                         current_param->param);
            return ACVP_INVALID_ARG;
            break;
        }
    ...
    while (current_param) {
        ...
        case ACVP_KTS_IFC_PARTIAL_VAL:
            ...
            break;
        default:
            ACVP_LOG_ERR("Unsupported KTS-IFC function param %d",
                         current_param->param);
            return ACVP_INVALID_ARG;
            break;
        }
    ...
    while (current_scheme) {
        ...
        if (current_param) {
            ...
            while (current_param) {
                ...
                case ACVP_KTS_IFC_RESPONDER:
                    ...
                    break;
                default:
                    ACVP_LOG_ERR("Unsupported KTS-IFC role param %d",
                                 current_param->param);
                    return ACVP_INVALID_ARG;
                    break;
                }
        ...
        json_object_set_boolean(meth_obj, "supportsNullAssociatedData",
                                current_scheme->null_assoc_data);
        if (current_scheme->assoc_data_pattern) {
            json_object_set_string(meth_obj, "associatedDataPattern",
                                   current_scheme->assoc_data_pattern);
        }
...
static ACVP_RESULT
acvp_build_safe_primes_register_cap(ACVP_CTX *ctx, JSON_Object *cap_obj,
                                    ACVP_CAPS_LIST *cap_entry) {
    JSON_Array *temp_arr = NULL;
    ACVP_RESULT result;
    const char *revision = NULL;
    ...
    if (current_genmeth) {
        while (current_genmeth) {
            ...
            case ACVP_SAFE_PRIMES_FFDHE8192:
                ...
                break;
            default:
                ACVP_LOG_ERR("Unsupported SAFE-PRIMES param %d",
                             current_genmeth->param);
                return ACVP_INVALID_ARG;
            }
...
static ACVP_RESULT acvp_build_lms_register_cap(ACVP_CTX *ctx,
    ...
    ACVP_RESULT result;
    const char *revision = NULL, *mode = NULL, *lms_str = NULL,
               *lmots_str = NULL;
    ...
    if (lms_cap->specific_list) {
        json_object_set_value(cap_obj, "specificCapabilities",
                              json_value_init_array());
...
static ACVP_RESULT acvp_build_ml_dsa_register_cap(ACVP_CTX *ctx,
    ...
    case ACVP_SUB_ML_DSA_KEYGEN:
        if (group_obj->param_sets) {
            json_object_set_value(cap_obj, "parameterSets",
                                  json_value_init_array());
            ...
            ACVP_LOG_ERR(
                "Missing required parameter sets for ML-DSA registration");
            goto err;
        }
        break;
    case ACVP_SUB_ML_DSA_SIGGEN:
    case ACVP_SUB_ML_DSA_SIGVER:
        if (alg == ACVP_SUB_ML_DSA_SIGGEN) {
            tf_val =
                ml_dsa_cap->deterministic; // Must change if
                                           // ACVP_DETERMINISTIC_MODE changes
            if (acvp_append_true_false_array(cap_obj, "deterministic",
                                             tf_val) != ACVP_SUCCESS) {
        ...
        json_object_set_value(cap_obj, "signatureInterfaces",
                              json_value_init_array());
        ...
        if (use_mu) {
            tf_val =
                ml_dsa_cap->mu - 1; // Must change if ACVP_ML_DSA_MU changes
            if (acvp_append_true_false_array(cap_obj, "externalMu", tf_val) !=
                ACVP_SUCCESS) {
        ...
        while (group_obj) {
            temp_val = json_value_init_object();
            temp_obj = json_value_get_object(temp_val);
            json_object_set_value(temp_obj, "parameterSets",
                                  json_value_init_array());
            ...
            json_object_set_value(temp_obj, "messageLength",
                                  json_value_init_array());
            ...
            json_object_set_number(temp2_obj, "increment",
                                   group_obj->msg_len.increment);
            ...
            if (group_obj->hash_algs) {
                json_object_set_value(temp_obj, "hashAlgs",
                                      json_value_init_array());
            ...
            if (group_obj->context_len.values ||
                group_obj->context_len.increment) {
                json_object_set_value(temp_obj, "contextLength",
                                      json_value_init_array());
                temp_arr = json_object_get_array(temp_obj, "contextLength");
                if (group_obj->context_len.increment) {
                    temp2_val = json_value_init_object();
                    temp2_obj = json_value_get_object(temp2_val);
                    json_object_set_number(temp2_obj, "min",
                                           group_obj->context_len.min);
                    json_object_set_number(temp2_obj, "max",
                                           group_obj->context_len.max);
                    json_object_set_number(temp2_obj, "increment",
                                           group_obj->context_len.increment);
                    json_array_append_value(temp_arr, temp2_val);
                }
...
static ACVP_RESULT acvp_build_ml_kem_register_cap(ACVP_CTX *ctx,
    ...
    if (alg == ACVP_SUB_ML_KEM_XCAP) {
        if (ml_kem_cap->functions) {
            json_object_set_value(cap_obj, "functions",
                                  json_value_init_array());
...
static ACVP_RESULT acvp_build_slh_dsa_register_cap(ACVP_CTX *ctx,
    ...
    case ACVP_SUB_SLH_DSA_KEYGEN:
        if (group_obj->param_sets) {
            ...
            ACVP_LOG_ERR(
                "Missing required parameter sets for SLH-DSA registration");
            goto err;
        }
        break;
    case ACVP_SUB_SLH_DSA_SIGGEN:
    case ACVP_SUB_SLH_DSA_SIGVER:
        if (alg == ACVP_SUB_SLH_DSA_SIGGEN) {
            tf_val =
                slh_dsa_cap->deterministic; // Must change if
                                            // ACVP_DETERMINISTIC_MODE changes
            if (acvp_append_true_false_array(cap_obj, "deterministic",
                                             tf_val) != ACVP_SUCCESS) {
...
ACVP_RESULT acvp_build_registration_json(ACVP_CTX *ctx, JSON_Value **reg) {
    ...
    if (ctx->caps_list) {
        ...
        while (cap_entry) {
            ...
            case ACVP_AES_GMAC:
                skip_final_append = 1;
                // Can have multiple vector sets; one for each IV mode, and one
                // for each IV source
                for (i = 0; i < ACVP_SYM_CIPH_IVGEN_MODE_MAX; i++) {
                    ...
                    if (cap_entry->cap.sym_cap->iv_mode_matrix[i][0] == 1) {
                        ...
                        // If src is either, build internal vector set
                        // registration, than external
                        if (cap_entry->cap.sym_cap->iv_mode_matrix[i][1] ==
                            ACVP_SYM_CIPH_IVGEN_SRC_EITHER) {
                            cap_entry->cap.sym_cap->iv_src =
                                ACVP_SYM_CIPH_IVGEN_SRC_INT;
                            rv = acvp_build_sym_cipher_register_cap(cap_obj,
                                                                    cap_entry);
                            if (rv != ACVP_SUCCESS) {
                                break;
                            }
                            ...
                            cap_entry->cap.sym_cap->iv_src =
                                cap_entry->cap.sym_cap->iv_mode_matrix[i][1];
                            rv = acvp_build_sym_cipher_register_cap(cap_obj,
                                                                    cap_entry);
                        }

                        // Since we aren't sure how many appends we are doing,
                        // just append here and skip the final
                        json_array_append_value(caps_arr, cap_val);
                        cap_val = NULL;
                    }
                }
            ...
            case ACVP_DSA_KEYGEN:
                rv = acvp_build_dsa_register_cap(cap_obj, cap_entry,
                                                 ACVP_DSA_MODE_KEYGEN);
                break;
            ...
            case ACVP_ECDSA_KEYVER:
                rv = acvp_build_ecdsa_register_cap(ctx, cap_entry->cipher,
                                                   cap_obj, cap_entry);
                break;
            case ACVP_ECDSA_SIGGEN:
                // If component_test = BOTH, we need two registrations
                if (cap_entry->cap.ecdsa_siggen_cap->component ==
                    ACVP_ECDSA_COMPONENT_MODE_BOTH) {
                    cap_entry->cap.ecdsa_siggen_cap->component =
            ...
            case ACVP_DET_ECDSA_SIGGEN:
                // If component_test = BOTH, we need two registrations
                if (cap_entry->cap.det_ecdsa_siggen_cap->component ==
                    ACVP_ECDSA_COMPONENT_MODE_BOTH) {
                    cap_entry->cap.det_ecdsa_siggen_cap->component =
                        ACVP_ECDSA_COMPONENT_MODE_NO;
                    rv = acvp_build_ecdsa_register_cap(ctx, cap_entry->cipher,
                                                       cap_obj, cap_entry);
                    if (rv != ACVP_SUCCESS) {
            ...
            case ACVP_KDF108:
                ...
                if (cap_entry->cap.kdf108_cap->kmac_mode.kdf_mode) {
                    rv =
                        acvp_build_kdf108_kmac_register_cap(cap_obj, cap_entry);
                    if (rv != ACVP_SUCCESS) {
                        break;
                    }
            ...
            case ACVP_KAS_ECC_CDH:
                rv = acvp_build_kas_ecc_register_cap(ctx, cap_obj, cap_entry,
                                                     ACVP_KAS_ECC_MODE_CDH);
                break;
            case ACVP_KAS_ECC_COMP:
                rv = acvp_build_kas_ecc_register_cap(
                    ctx, cap_obj, cap_entry, ACVP_KAS_ECC_MODE_COMPONENT);
                break;
            ...
            case ACVP_KAS_FFC_COMP:
                rv = acvp_build_kas_ffc_register_cap(
                    ctx, cap_obj, cap_entry, ACVP_KAS_FFC_MODE_COMPONENT);
                break;
            case ACVP_KAS_FFC_NOCOMP:
                rv = acvp_build_kas_ffc_register_cap(ctx, cap_obj, cap_entry,
            ...
            case ACVP_KDA_ONESTEP:
                rv = acvp_build_kda_onestep_register_cap(ctx, cap_obj,
                                                         cap_entry);
                break;
            case ACVP_KDA_TWOSTEP:
                rv = acvp_build_kda_twostep_register_cap(ctx, cap_obj,
                                                         cap_entry);
                break;
            case ACVP_KDA_HKDF:
            ...
            case ACVP_SAFE_PRIMES_KEYVER:
                rv = acvp_build_safe_primes_register_cap(ctx, cap_obj,
                                                         cap_entry);
                break;
            ...
            if (rv != ACVP_SUCCESS) {
                ACVP_LOG_ERR("failed to build registration for cipher %s (%d)",
                             acvp_lookup_cipher_name(cap_entry->cipher), rv);
                json_value_free(cap_val);
                json_value_free(val);
                return rv;
            }
...
ACVP_RESULT acvp_build_full_registration(ACVP_CTX *ctx, char **out,
                                         int *out_len) {
...
ACVP_RESULT acvp_build_validation(ACVP_CTX *ctx, char **out, int *out_len) {
    ...
    if (oe->url) {
        ...
        if (oe->dependencies.status == ACVP_RESOURCE_STATUS_COMPLETE ||
            ...
            json_object_set_value(oe_obj, "dependencyUrls",
                                  json_value_init_array());
        ...
        if (oe->dependencies.status == ACVP_RESOURCE_STATUS_INCOMPLETE ||
            ...
            json_object_set_value(oe_obj, "dependencies",
                                  json_value_init_array());
            ...
            for (i = 0; i < oe->dependencies.count; i++) {
                ...
                if (dependency->url == NULL) {
                    ...
                    if (dependency->type) {
                        json_object_set_string(dep_obj, "type",
                                               dependency->type);
                    }
                    if (dependency->name) {
                        json_object_set_string(dep_obj, "name",
                                               dependency->name);
                    }
                    if (dependency->description) {
                        json_object_set_string(dep_obj, "description",
                                               dependency->description);
                    }
                    if (dependency->version) {
                        json_object_set_string(dep_obj, "version",
                                               dependency->version);
                    }
                    if (dependency->family) {
                        json_object_set_string(dep_obj, "family",
                                               dependency->family);
                    }
                    if (dependency->series) {
                        json_object_set_string(dep_obj, "series",
                                               dependency->series);
                    }
                    if (dependency->manufacturer) {
                        json_object_set_string(dep_obj, "manufacturer",
    ...
    if (module->url) {
        ...
        if (module->description) {
            json_object_set_string(module_obj, "description",
                                   module->description);
        }

        json_object_set_string(module_obj, "vendorUrl", module->vendor->url);
        json_object_set_string(module_obj, "addressUrl",
                               module->vendor->address.url);

        json_object_set_value(module_obj, "contactUrls",
                              json_value_init_array());
